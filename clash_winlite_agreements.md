...
2. **滑出设置面板**：
   ...
   * 设置面板内采用**左右分栏**布局：
      * **左侧分栏（固定为 200px 宽度）**：自上而下整齐摆放「机场订阅区」、「接管模式选择」与「极简设置项」，不随窗口宽度变化拉伸。其中，为了解决标题字号反而显得小的问题，抽屉内所有设置项标签（如“开机自动启动”、“启动时最小化”、“通知弹窗显示”、“主题模式”、“代理策略”、“Mixed Port”等）的字号一律统一为 `11px`，与各自模块标题字号（`11px`）保持一致。同时，为了使三个开关按钮（开机自动启动、启动时最小化、通知弹窗显示）与小文字视觉比例协调，Switch 控件采用 `transform: 'scale(0.75)'` 进行整体缩小，并采用 `transformOrigin: 'right center'` 靠右对齐。
...
9. **主页专属表格：主界面节点列表（三列表格）网格设计**：
    ...
    * 列间纵向双线划分：为突出三列排版的对称美感，在第一列右侧、第二列右侧的中缝，使用宽度为 `5px double` 的 vertical 双实线边框（`(theme) => 5px double ${theme.palette.divider}`），实现明显的隔断感。
    * 延迟列居中对齐与宽度：延迟数值那一列（每个节点的最右侧）的宽度由原本的 `50px` 调整为 `65px`，以完全容纳 `"Timeout"` 和 `"testing"` 等长文本，对齐方式改为居中对齐（`justifyContent: "center"`）。
    * 外周双框线包边：整个节点列表表格的外部容器竖边框（左、右竖外边框）线宽设置为 **`5px double`** 双实线（使用 `theme.palette.divider`），上、下横外边框为 `4px solid`，形成精致的包边效果。
...
4. **设置页专属表格：手动路径控制（连接管理）双列设计与滚动条高度约束**：
    ...
    * **表格风格与双框对称线**：超紧凑 Excel 风格，行高固定为 `20`，单元格 py 内边距收窄为 `0.5`，文字字号 `12px`，单元格左右边距 `1.5`。外周包裹边框线由 solid 升级为四周统一的 **`5px double`** 双实线（基于 `theme.palette.divider`），标题行与数据行之间的分割线也同步重构为 **`5px double`** 双实线。单元格内部行/列网格分割线为 `2px solid` 实体分割格线。奇偶行使用淡色调的斑马纹。
