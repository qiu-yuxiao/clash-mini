# Clash WinAero Memory Checkpoint (记忆整理归档)
...
1. **毛玻璃与半透明彻底清除 (100% Solid Colors)**
   * **修改**：在全局 CSS (`layout.scss`)、前端组件 (`_layout.tsx`, `connection-table.tsx` 等) 以及 Rust 后端窗口配置 (`window.rs`) 中，完全移除了所有毛玻璃滤镜 (`backdrop-filter`) 并将半透明背景提升为 100% 实体纯色（如亮色 `#ffffff`/`#f0f5ff`，暗色 `#1e2438`/`#0f1423`）。
...
* **隔离策略**：仅能通过 `manage_task` 杀掉 `c453492a-295b-4505-ab6d-1555ef293576/task-7731` 对应的开发版 Tauri 进程，或者针对开发版的前端/后端进程进行精准清理。

---

## 🔍 启动前安全参数验证 (Config Validation)
我们已核实了开发版 AppData 配置（[verge.yaml](file:///C:/Users/sun_y/AppData/Roaming/io.github.clash-winaero.clash-winaero.dev/verge.yaml) 和 [config.yaml](file:///C:/Users/sun_y/AppData/Roaming/io.github.clash-winaero.clash-winaero.dev/config.yaml)）：
...
2. **WebView2 用户数据缓存 (UDF)**：本地的 `C:\Users\sun_y\AppData\Local\io.github.clash-winaero.clash-winaero` 目录（包含渲染器缓存、GPU 状态、Shader 编译等缓存）。

---

## 🔄 重新启动指南 (Dev Startup Guide)
1. **关闭旧任务**：调用 `manage_task` 的 `kill` 操作，终止正在运行 of `c453492a-295b-4505-ab6d-1555ef293576/task-7731` 任务。
2. **清理缓存**：在工作目录下运行 Powershell 缓存清理命令：
   ```powershell
   Remove-Item -Recurse -Force node_modules/.vite
   Remove-Item -Recurse -Force C:\Users\sun_y\AppData\Local\io.github.clash-winaero.clash-winaero
   ```
