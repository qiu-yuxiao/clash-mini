...
### **BUG-065** (主页活跃出口节点即刻轮换)
* **缺陷描述与现象**：在微缩窗口下，需要一种一键更换活动节点的方法（点击节点名直接强制更换为当前过滤/排序后子集里的另一个节点）。
* **排查原因与记忆**：因为出口节点列表为虚拟滚动，DOM 中无完整节点，且需要保持筛选和排序顺序。因此在顶部状态卡片内重新从 `localStorage` 解析 `proxy-head-state` 得到过滤和排序配置，并调用 `filterSort` 得到可视节点子集数组进行轮换。
* **修改方针**：重构 `_layout.tsx`，在 ActiveNodeStatusCard 中实现该子集计算与 handleCycleNode 点击轮换动作，包装 Tooltip 提示并加 `cursor: 'pointer'` 及 hover 样式。状态目前为：`代码已修正，待确认`。
