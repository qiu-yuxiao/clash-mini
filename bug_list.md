...
### BUG-123: 快速切换配置时 catch 逻辑相互覆盖引发 SWR 重复请求与重?

- **现象描述**：用户在侧边栏快速连续切换多个配置文件时，界面出现明显卡顿，且后台会触发多次重复的增强配置操作?
- **根因**：较旧的 `enhanceProfiles` Promise 失败被清理时，其 `.catch()` 依然会同步修?`lastEnhancedProfileRef.current = null`。这覆盖了新切换配置所写入?Ref 值，造成组件重新渲染时重复编译?
- **修正说明**：在 `.catch` 异常捕获块中添加 `if (cancelled) return` 过滤，避免旧异步任务清理掉新任务的引用缓存?
- **当前状?*：`代码已修正，待用户确认`
- **目标版本**：`v1.4.8`

### BUG-124: 活跃节点状态卡片频繁切换节点名称时产生内存泄漏与竞态覆?

- **现象描述**：节点切换时，活跃出口节点状态卡片中显示的地址（IP:Port）偶尔会在极短时间内跳变回上一个节点的地址?
- **根因**：`ActiveNodeStatusCard` 在执?`getProxyAddr` 异步查询时没有添?cleanup 回调，导致旧请求晚于新请求返回时覆盖了最新节点的地址。此外，在组件卸载后调用状态更新会导致 React 报错?
- **修正说明**：在 `ActiveNodeStatusCard` ?`useEffect` 中注?`cancelled` 状态机，并在清除函数中将其设为 `true`，以舍弃过时的查询返回?
- **当前状?*：`代码已修正，待用户确认`
- **目标版本**：`v1.4.8`

### BUG-125: 主题参数调节滑动条边界范围未遵守各皮肤的特定设计规范

- **现象描述**：切换到不同主题皮肤时，参数调节滑动条的最小值和最大值没有改变，例如?Monochrome ?Contrast 可以滑动?0.0，导致文字完全不可见?
- **根因**：`theme-settings-card.tsx` 中滑动条 `min` ?`max` 被硬编码或使用了错误的固定计算，未与 Trump-3D/Original/Monochrome 等六套皮肤的专属上下限对齐?
- **修正说明**：实?`getSlider1Min`、`getSlider1Max` ?`getSlider2Max` 边界值辅助函数，在不同皮肤下动态分配专属的边界范围（例?Original ?Radius 限制?0.0 - 5.0，Monochrome ?Contrast 限制?0.3 - 1.0）?
- **当前状?*：`代码已修正，待用户确认`
- **目标版本**：`v1.4.8`

### BUG-126: 弹出菜单与对话框层级透明背景导致文字重叠不可?

- **现象描述**：在打开编辑订阅对话框、核?客户端更新对话框、以及节点或导入框的右键上下文菜单时，弹窗或菜单背景呈完全透明，导致弹窗内文字与底层界面的文字交错重叠，完全无法阅读?
- **根因**：虽然弹窗和菜单容器均附加了 `className="theme-panel"` 类，?inline `sx` 属性中被硬编码?`backgroundColor: 'transparent'`，以最高优先级覆盖了皮肤类定义的实体背景色?
- **修正说明**：移?Dialog ?Menu 关联 `paper` 槽中 `sx` 内的 `backgroundColor: 'transparent'` 覆盖，使其能够正常读取皮肤系统赋?`.theme-panel` 的非透明实体材质背景?
- **当前状?*：`代码已修正，待用户确认`
- **目标版本**：`v1.4.8`

### BUG-127: 底部流量指标卡片排列顺序不统一

- **现象描述**：下方流量小卡片的排列顺序为先“下载”后“上传”，与上方流量图图例中的“上传”在前、“下载”在后的先后次序相反，界面排列不统一?
- **根因**：`mini-traffic-panel.tsx` ?JSX 渲染树节点里 Download 相关的卡片组直接被定义在?Upload 卡片组之前?
- **修正说明**：调?JSX 渲染树中 Upload ?Download 相关 Box 容器的先后位置，使其符合“上传在前、下载在后”的规范?
- **当前状?*：`代码已修正，待用户确认`
- **目标版本**：`v1.4.8`

### BUG-128: 皮肤风格分段选择器渲染高频触?LocalStorage 同步读取

- **现象描述**：在切换选项或操作主界面时，出现微小的操作迟滞，这是由于主渲染线程频繁发?LocalStorage 磁盘读取导致的?
- **根因**：分段选择器中传入了布尔值而非 `theme` 本身，导致辅助函?`get3DSegmentedContainerStyle` 降级读取 `localStorage`，在 React 重绘生命周期内执行了大量的同?I/O?
- **修正说明**：修改所有分段选择器的样式传入，将完整?`theme` 对象传递给 `get3DSegmentedContainerStyle`，以直接在内存读?skin 值，免除 LocalStorage 频繁存取?
- **当前状?*：`代码已修正，待用户确认`
- **目标版本**：`v1.4.8`

### BUG-129: 后台自愈优选检测因互斥原子锁忙而跳过配置重载切?

- **现象描述**：在某些瞬时高并发场景下切换配置，活跃出口节点会卡在上一配置文件中的老节点，自动选优静默跳过，不再生效?
- **根因**：当原子?`AUTO_SELECT_RUNNING` 繁忙时，`trigger_backend_auto_select` 会直接返?`Ok(vec![])`。后?monitor 检测到 profile 变动后自愈测速因此跳过，且记录了 `last_profile_uid` 导致未来不再尝试?
- **修正说明**：锁冲突时返?`AUTO_SELECT_BUSY` 错误，同时在后台 monitor 检测到该错误时实施带延时的自旋重试机制，确保其最终完成?
- **当前状?*：`代码已修正，待用户确认`
- **目标版本**：`v1.4.8`

### BUG-130: 测速排序类型为 None 时忽略本地已存配置直接写死为 1

- **现象描述**：手动测速排序在未显式指定方式时，无法记住或应用用户之前?`proxy_head_state.json` 中保存的自定义排序类型?
- **根因**：`trigger_auto_select` Tauri 接口中，对于可选参?`sort_type` ?`None` 的情况使用了 `.unwrap_or(1)`，直接将类型固定?`1`（按延迟），使得 `monitor.rs` 内部针对 `0`（从配置文件读取）的逻辑无法被触发?
- **修正说明**：接口处将其修改?`.unwrap_or(0)`，使其在未传参时正确路由到读取本地已存的排序状态逻辑?
- **当前状?*：`代码已修正，待用户确认`
- **目标版本**：`v1.4.8`

### BUG-131: 后台读取保存排序类型时使用效率低下的 YAML 解析器解?JSON

- **现象描述**：后台监测服务在读取 `proxy_head_state.json` 文件时，有不必要?CPU 时间开销?
- **根因**：`get_saved_sort_type` 函数中使用了 `serde_yaml_ng::from_str` 来解?JSON 配置文件?
- **修正说明**：将解析器函数统一更换为原生的 `serde_json::from_str` 专用反序列化器，提升执行效率?
- **当前状?*：`代码已修正，待用户确认`
- **目标版本**：`v1.4.8`
...
### BUG-133: 异常回滚恢复?Profile 时未触发核心重载导致配置脱节

- **现象描述**：当新导入的配置文件发生校验失败触发自愈回滚时，程序提示恢复成功，但内核实际上仍然在尝试解析失效/损坏的配置?
- **根因**：`restore_previous_profile` 在写入回滚后?profile uid 后，没有调用 `CoreManager::global().update_config_forced()` 强制 core 重新加载?
- **修正说明**：在异步恢复配置工作区线程的结尾处，补充强制重载 core 的调用，保证配置与内核绝对对齐?
- **当前状?*：`代码已修正，待用户确认`
- **目标版本**：`v1.4.8`
...
### BUG-135: 检?DNS 配置文件是否存在命令同步阻塞 UI 主线?

- **现象描述**：打开设置或触?DNS 检查时，窗口界面偶现极短时间的无响应和瞬间白屏?
- **根因**：`check_dns_config_exists` 为同步命令函数，Tauri 将其放在 UI/主事件循环线程中处理，同步磁?I/O（`exists()`）会导致界面事件响应挂起?
- **修正说明**：将?Tauri 命令声明修改?`async fn`，由 Tauri 路由?Tokio 异步线程池执行，确保主线程响应绝对平滑?
- **当前状?*：`代码已修正，待用户确认`
- **目标版本**：`v1.4.8`
