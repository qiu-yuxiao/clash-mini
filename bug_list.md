...
* **排查原因与记忆**：在 Windows 管理员模式下，启动初期主窗口处于隐藏且未加载状态，此时主线程向 Explorer 发送托盘更新指令容易被阻断。且 `.ico` 文件大小较小可能包含不规范图层导致底层 `muda` 库 `set_icon` 失败。
* **修改方针**：在 Windows 平台下将默认图标全面换用 Tauri v2 完美支持的 `.png` 格式；同时为托盘图标和提示语更新引入后台非阻塞式（异步 channel + sleep）重试机制，避开系统启动卡顿。
* **状态**：代码已修正，待确认。
