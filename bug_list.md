...
### **BUG-082** (WebView2 后台挂机内存占用过高)
* **缺陷描述与现象**：程序长时间在后台托盘运行或最小化时，WebView2 浏览器内核没有进行自动内存回收，物理内存占用较多。
* **排查原因与记忆**：
  1. WebView2 默认在后台会保留全部渲染页面、缓存以及进程上下文；
  2. Rust 侧在监听到窗口最小化或隐藏到系统托盘时，需要显式触发 WebView2 的内存回收。
* **修改方针**：在 Rust 侧 `lib.rs` 的窗口隐藏/最小化/失去焦点事件中，通过调用 WebView2 实例对象的内存回收逻辑释放闲置资源。
* **状态**：代码已修正，待用户确认。

### **BUG-093** (启动或更新后自动连切节点延迟过高)
* **缺陷描述与现象**：Clash Mini 启动或重新加载配置后，节点自动激活与连接存在约 1.5 到 3 分钟的显著延迟，用户无法立刻自动接通可用节点。
* **排查原因与记忆**：
  1. 启动及重载时，前端首次全局测速与 Clash 内核冷启动冲突，在网络尚未完全初始化时强行测速导致节点健康探测全部超时；
  2. 首次测速失败后，后台监控器具有 60 秒等待时间，且在更新订阅后恢复历史选点失效。
* **修改方针**：设计双轨启动与连接恢复方案，包括模糊匹配历史可用节点（忽略时间戳后缀）直接信任接通，以及优化首次冷启动下的探活就绪信号判定。
* **状态**：代码已修正，待用户确认。
