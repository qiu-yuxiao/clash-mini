...
### **BUG-091** (订阅链接/节点配置输入框无法使用鼠标右键粘贴等右键菜单操作)
* **缺陷描述与现象**：在设置面板的机场订阅配置输入框中，用户点击鼠标右键无法弹出右键菜单，导致只能使用键盘 `Ctrl+V` 进行粘贴，无法使用鼠标右键进行粘贴、复制、剪切、全选等操作，极大影响交互体验。
* **排查原因与记忆**：由于 Tauri 默认禁用了 WebView2 生产环境的网页原生右键菜单以防止 inspect 等调试操作，导致输入框失去了默认的粘贴/复制菜单。
* **修改方针**：在机场订阅输入框上绑定 `onContextMenu` 事件并组织默认行为，然后通过 MUI 的 `<Menu>` 弹窗自行实现一个针对该输入框的拟物右键菜单，调用 `@tauri-apps/plugin-clipboard-manager` 提供的接口执行剪切（Cut）、复制（Copy）、粘贴（Paste）、全选（Select All）与清空（Clear）等输入操作，并在粘贴时精确判断光标所处选区（Caret Selection）以符合用户原生的粘贴期望。
* **状态**：代码已修正，待确认。

### **BUG-092** (手动触发全体测速完成后无法自动切换到过滤子集中的最快节点)
* **缺陷描述与现象**：在主页节点列表表头点击“闪电光标”触发全体测速时，测速完成后只会刷新延迟并重新排序（若开启了排序），但不会自动将出口节点切换到测速后最快的那一个健康节点，用户仍需手动寻找并点击，体验不连贯。
* **排查原因与记忆**：目前 `handleCheckAll` 测速函数的 `finally` 逻辑只更新了测试状态并刷新了节点列表（调用 `onProxies`），未主动计算过滤子集中的最小正延迟节点并执行 `changeProxy` 切换动作。
* **修改方针**：在测速完成后，提取当前过滤子集（即 `proxies` 列表），使用 `delayManager.getDelayFix` 遍历找出其中延迟大于 0 且小于 timeout 的最小延迟节点。若存在，则自动调用 `changeProxy` 将当前活跃节点切换至该最快节点。
* **状态**：排查中。

### **BUG-090** (Trump-3D 皮肤深色模式下设置模组与出口节点栏文字看不清)
* **缺陷描述与现象**：在 Trump-3D (retro-3d) 皮肤的深色（Dark）模式下，左侧设置面板的三个卡片（“订阅与机场配置”、“流量接管模式”、“基础设置”）以及顶部的当前活跃出口节点表示框，其文字（标题、标签、数值、以及非选中的分段按钮文字等）默认渲染为白色或浅灰色，与金黄色/橙色的亮色渐变背景严重混色粘连，极难看清。
* **排查原因与记忆**：MUI 文本组件（如 Typography）在深色模式下默认继承白色系的 `text.primary` / `text.secondary`，未在金色这种偏亮色背景上进行动态高反差覆盖。
* **修改方针**：在 `_layout.tsx` 中识别 `isRetro3DDark` 条件（即 `skin === 'retro-3d' && theme.palette.mode === 'dark'`），将这些静态文本强制设为最高对比度的深古铜黑色 `#2C1F03`；对于点击交互元素（如活跃节点名称、未选中的分段按钮文字），在悬停 (hover) 及按下 (active) 状态分别提供 `#664600` / `rgba(44, 31, 3, 0.65)` / `rgba(44, 31, 3, 0.45)` 等自然的明暗变化反馈。
* **状态**：代码已修正，待确认。
