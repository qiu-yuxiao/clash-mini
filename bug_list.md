...
### **BUG-073** (Windows系统托盘图标及提示语未显示)
* **缺陷描述与现象**：在 Windows 开发或便携运行环境下，启动后系统托盘不显示程序的图标，且日志中不断报出 `os error -2147467259` (未指定的错误)。
* **排查原因与记忆**：系统托盘的创建与更新操作被调度在 Tokio 异步后台工作线程（如 `clash-mini-runtime-0`）中执行，而 Windows 的 Win32 托盘图标关联窗口具有强烈的线程亲和性（Thread Affinity），在没有 GUI 消息泵的后台线程更新会导致 Windows Shell API 报错 `E_FAIL`。
* **修改方针**：将系统托盘的初始化 `init` 和更新 `update` 逻辑通过 `run_on_main_thread` 调度回主线程（`T{main}`）执行。
* **状态**：排查中。
...
| **BUG-072** | 新版内核插件序列化问题导致 Allow LAN 等开关状态无法加载与更新。 | v1.1.8 | 代码已修正，已确认 |
