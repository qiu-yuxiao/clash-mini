...
### BUG-257: Active Node Health Check CPU and Battery Overhead in Lightweight Mode
 - **现象描述**：在轻量模式下，主窗口已销毁，程序处于后台托盘静默运行状态，但常驻监测线程依然以 15 秒的固定高频频率发起活跃节点延迟探测和 DNS 解析，在长期无人值守或电池供电状态下产生了不必要的 CPU 开销、网络流量消耗和电量损耗。需要实现自适应检测周期：处于轻量模式时将检测周期延长至 60 秒以节省资源，当退出轻量模式还原窗口时立即可靠恢复至 15 秒以保证用户体验。
 - **当前状态**：`代码已修正，待用户确认`
 - **目标版本**：`v1.8.10`
