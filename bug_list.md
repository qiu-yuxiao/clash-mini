...
### **BUG-085** (自动测速检测节点健康时，偶现或频现“所有线路都繁忙”误报弹窗)
* **缺陷描述与现象**：在程序运行或自动刷新检测节点健康时，即使存在大量 100ms 左右的可用节点，依然会频繁弹出“所有线路都繁忙，请耐心等待”的红色错误提示。
* **排查原因与记忆**：
  1. **后台静默缺失**：在 `triggerAutoSelectFastestNode` 中，错误通知 `showNotice.error` 未进行 `!isBackground` 的判断，导致后台自动检测失败时也会弹出前台打扰通知。
  2. **状态抢跑与测试中误判**：Clash/Mihomo 内核在触发 Provider 健康检查时会立即清空历史（返回 `delay: 0` 即 `1e6`）；而前端 `delayManager` 对非 Provider 节点在测试开始时也会设为 `-2`（代表“测试中”）。但在自动选点轮询逻辑中，任何非 `-1`（包括 `-2` 和 `1e6`）的延迟均被错误地累加到 `testedCount`（已测数量）中。这导致在测速开始的 500 毫秒（第一次轮询），`testedCount` 就已经达到了节点总数，触发了 `isFinalSelection` 的立刻退出条件，未等真实测速数据返回就报错退出。
* **修改方针**：
  1. 对 `showNotice.error` 加上 `if (!isBackground)` 包装，确保后台静默执行。
  2. 修复 `isFinalSelection` 的退出条件，只有当测速完成且 `healthyNodes.length >= 1` 时才允许通过 `testedCount` 提前退出；如果暂无健康节点，强制轮询至少等待 6 秒（或直到 15 秒超时），等待 Clash 核心及前端测速写入真实的延迟数据。
* **状态**：代码已修正，待确认。

### **BUG-086** (窄窗口流量曲线图频现锯齿状大幅度跌落至零的问题)
* **缺陷描述与现象**：在窄窗口模式下，底部的流量图在拉伸或挂机运行时，数据点频繁跌落为 0，曲线出现严重的锯齿状断裂，无法显示平滑的实时流量。
* **排查原因与记忆**：
  1. 数据流源头 `useTrafficData` 中的 WebSocket 消息使用了 `throttleMs: 3000` 进行了 3 秒的限流。
  2. 窄窗口特有的渲染组件 `TrafficGraph` 的数据消费采样频率却设为 `1000ms`（1秒），且在读取 `cacheRef.current` 后立刻清空它。这导致每 3 秒内，只有第 1 秒能取到有效流量，剩下 2 秒全部因为清空而读取到 `zeroTraffic` (0)。
* **修改方针**：执行方案 B，将 `useTrafficData` 的 `throttleMs` 从 `3000` 修改为 `1000`（1秒），使其与前端 canvas 绘制的 1 秒频率对齐，解决锯齿掉零的现象。
* **状态**：代码已修正，待确认。

### **BUG-087** (将 Retro-3D 重构为 Trump-3D 黄金块拟物皮肤)
* **缺陷描述与现象**：将现在的 "Retro-3D" 风格的那个按钮的表示名改为 "Trump-3D"，并将整个风格的颜色调整得更像是黄金块，让那些按钮都像是金条金块摆在那里的样子。
* **排查原因与记忆**：原 "Retro-3D" 皮肤在展示层使用了灰银色/白银色（Titanium Silver）和橙黄色渐变，不符合金条金块的奢华重质感拟物设计。
* **修改方针**：
  1. 在 `_layout.tsx` 中将换肤按钮的 label 改为 'Trump-3D'。
  2. 重构 `button-styles.ts` 下的按钮、卡片、输入框、分段选择等样式，应用高反光多色标黄金线性渐变背景、古铜描边和重质感物理按压反馈（凹版文字阴影和重阻尼按压）。
  3. 重绘全局面板、卡片、Switch 和 Slider，升级为金条高光渐变与双实线黄金边框。
* **状态**：排查中。
