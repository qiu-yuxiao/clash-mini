...
| :--- | :--- | :---: | :--- |
| **BUG-015** | 配置文件增强与激活逻辑在 `_layout` 的 `useEffect` 中与未缓存的 `activateSelected` 发生联动，导致启动和重载后陷入无限循环重新加载并弹窗报错“配置校验失败”。 | v1.0.5 | 从 `_layout.tsx` 的 `useEffect` 依赖项中移除 `activateSelected`，仅保留 `currentProfileUid`，并在行尾添加 eslint 禁用注释，彻底斩断无限更新环。 |
