...
fn parse_vless(link: &str) -> Option<serde_yaml_ng::Mapping> {
    ...
    map.insert(
        serde_yaml_ng::Value::from("type"),
        serde_yaml_ng::Value::from("vless"),
    );
    map.insert(
        serde_yaml_ng::Value::from("name"),
        serde_yaml_ng::Value::from(remarks),
    ...
    if let Some(q) = query {
        ...
        if !network_type.is_empty() {
            let net_val = if network_type == "raw" { "tcp".to_string() } else { network_type };
            map.insert(
                serde_yaml_ng::Value::from("network"),
            ...
            if net_val == "ws" {
                ...
                if !host_str.is_empty() {
                    ...
                    headers.insert(
                        serde_yaml_ng::Value::from("Host"),
                        serde_yaml_ng::Value::from(host_str),
                    );
                    ws_opts.insert(
                        serde_yaml_ng::Value::from("headers"),
                        serde_yaml_ng::Value::from(headers),
...
mod tests {
    ...
    fn test_parse_vless_reality() {
        ...
        assert_eq!(map.get(&serde_yaml_ng::Value::from("type")).unwrap().as_str().unwrap(), "vless");
        assert_eq!(map.get(&serde_yaml_ng::Value::from("name")).unwrap().as_str().unwrap(), "aws德国");
        assert_eq!(map.get(&serde_yaml_ng::Value::from("server")).unwrap().as_str().unwrap(), "awsjfde1.fatdns.net");
        assert_eq!(map.get(&serde_yaml_ng::Value::from("port")).unwrap().as_u64().unwrap(), 443);
        assert_eq!(map.get(&serde_yaml_ng::Value::from("uuid")).unwrap().as_str().unwrap(), "2513e61a-6982-39cd-9f0a-8f6fbc3fc5f0");
        assert_eq!(map.get(&serde_yaml_ng::Value::from("flow")).unwrap().as_str().unwrap(), "xtls-rprx-vision");
        assert_eq!(map.get(&serde_yaml_ng::Value::from("tls")).unwrap().as_bool().unwrap(), true);
        assert_eq!(map.get(&serde_yaml_ng::Value::from("servername")).unwrap().as_str().unwrap(), "s0.awsstatic.com");
        assert_eq!(map.get(&serde_yaml_ng::Value::from("client-fingerprint")).unwrap().as_str().unwrap(), "chrome");
        
        let reality_opts = map.get(&serde_yaml_ng::Value::from("reality-opts")).unwrap().as_mapping().unwrap();
        assert_eq!(reality_opts.get(&serde_yaml_ng::Value::from("public-key")).unwrap().as_str().unwrap(), "QzUWMFvn-J14NCZGsn5pz30inE7VT9auYKRwaRr5T20");
        assert_eq!(reality_opts.get(&serde_yaml_ng::Value::from("short-id")).unwrap().as_str().unwrap(), "00fd4e9ac051c144");
    }
    ...
    fn test_parse_http_fallback() {
        ...
        assert_eq!(map.get(&serde_yaml_ng::Value::from("type")).unwrap().as_str().unwrap(), "http");
        assert_eq!(map.get(&serde_yaml_ng::Value::from("name")).unwrap().as_str().unwrap(), "v5-香港03|1x|v");
        assert_eq!(map.get(&serde_yaml_ng::Value::from("server")).unwrap().as_str().unwrap(), "v5.cdn.ljz8s7lhbv.fatdns.net");
        assert_eq!(map.get(&serde_yaml_ng::Value::from("port")).unwrap().as_u64().unwrap(), 6712);
    }
}
