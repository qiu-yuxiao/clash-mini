...
- **同步防阻塞机制**：若是管理员运行，必须立即返回跳过 5 秒的异步服务重试轮询。这直接保证了初始化任务 of 无延迟顺序执行，确保内核在主线程发起配置重载（`reload_config`）之前已经就绪，进而消除因管道连接失败导致重复触发“错误重启拉起”而形成的双进程内核残留泄漏。
...
- **废除运行期自动版本比对与重装**：为了彻底消除与原版客户端的 SCM 服务抢占纠葛，运行时（包括 `refresh` 后台刷新）禁止对系统服务进行任何版本比对，不得自动发起 `NeedsReinstall` 等静默提权卸载与重装行为。如果服务已安装但停止，仅允许触发启动服务操作（StartRequired）。
...
3. **窗口装饰与标题栏**：
   ...
   * **标题栏行为规范（FEAT-003）**：
     ...
     * **恢复方式1（单击唤醒）**：在窗口上释放鼠标（即触发 `mouseup` 且鼠标位移量小于 5 像素，且本次按压未曾启动过拖拽）→ 显示自定义标题栏，GlowBorder 消失。排除标记为 `data-no-drag="true"` 的避让区域，点击避让区域仅执行原有功能而不触发唤醒。

     * **恢复方式2（变宽）**：窗口被拖宽脱离最小宽度（innerWidth > 285px）→ 自动显示自定义标题栏，GlowBorder 消失。
     ...
     * **最小尺寸强制约束**：由于 Tauri v2 frameless 窗口的 `startResizeDragging` API 不自动遵守 `min_inner_size` 限制，必须在 `on_window_event` 的 `Resized` 事件中手动校验并强制纠正窗口尺寸。最小化和最大化状态下跳过检查。
...
8. **基础设置模块排版与外观主题**：
   ...
   * **界面华丽度双控制系统（双滑动条）**：在“主题模式”下方增加两个滑动条组件（Mui Slider），标签分别为“立体磨砂 (Depth)”与“色彩霓虹 (Vibrancy)”。数值范围和上限根据不同皮肤在前端代码中进行动态定义（步长为 `0.1`，默认值 `1.0`），数据在 LocalStorage 中持久化保存，并在 React 中监听变化，通过 `document.documentElement.style.setProperty` 将变量 `--depth-factor` and `--vibrancy-factor` 动态注入至最顶层 `:root`，用以全局联动控制光影与发光效果。
...
- **精确防丢包抖动的自愈时间参数**：
    - **检测周期与分级门控**：为了取得实时网络响应和极佳的系统资源/功耗控制的完美平衡，常规监测周期调整为 **`60秒`**；当程序被关闭到系统托盘进入轻量模式时，监测周期自动放宽到 **`300秒（5分钟）`** 以避免背景空转；当疑似故障进入重试阶段时，周期维持在 **`3秒`**。
    - **10分钟切换冷却限制 (手动与自动统一冷却)**：不管是自动选点触发的节点切换，还是手动在 UI 界面触发的节点切换，一旦发生节点切换，在接下来的 **`10分钟（600秒）`** 冷却时间内，自动健康监测**禁止**发起自动切换行为。唯一的例外是：当当前活跃节点发生 **`Dead`（彻底断线）** 即延迟测速超时或完全无连接返回时，允许无视 10 分钟冷却强制触发自愈选点以抢救网络；用户在 UI 界面发起的强制手动切换则始终不受此冷却限制。
  - **防抖自愈触发阈值**：必须连续 **`5次`** 检测失败才触发批量选点自愈。
  - **测速超时限制**：活跃节点单点检测超时设为 **`1000ms`**，延迟满足大于等于 `30ms` 且成功返回即为健康（后端 monitor.rs 统一判定 `delay >= 30`，与前端健康阈值完全一致，彻底消除单点健康状态的二义性）；全节点优选测速超时设为 **`2000ms`**，且有效延迟必须大于等于 `30ms`，快速剔除卡顿/死/高延迟节点。
- **网络连接的防死锁安全**：
...
- **网络状态感知与自愈冷却自动重置机制**：
  - **网络状态监测与转换**：后台监测服务必须具备定期评估系统网络连接状态的能力。当系统由离线（无网络连接）转换为在线（已连接到互联网）时，自愈系统必须被即时激活。
  - **自愈冷却强制重置**：在网络由离线恢复为在线的瞬间，程序必须无条件强制重置和清除之前因为冷启动断网或测试失败而触发的所有退避冷却限制与连续失败计数器。
  - **即时纠偏切换**：网络恢复后，如果当前活跃的代理节点处于不可用或不健康状态，程序必须立即并发测速并优选切换到健康的真实节点上，防止客户端因开机网络滞后而死锁在不可用的默认广告/假节点中。
...
不管是自动选点触发的节点切换，还是手动在 UI 界面触发的节点切换，一旦发生节点切换，在接下来的 **10 分钟（600 秒）** 冷却时间内，自动健康监测禁止发起自动切换。唯一的例外是当前活跃节点发生 Dead（彻底断线）时允许强制自愈。

**冷却计时器初始化**：`last_node_switch_time` 须初始化为 `Option<Instant>` 的 `None`。冷却仅在实际发生节点切换后才开始计时。启动后首次自愈选点不受冷却限制。

### 5.3 网络状态感知与在线恢复

后台监测服务须定期评估系统网络连接状态。当系统由离线转为在线时，自愈系统须即时激活，无条件重置所有退避冷却限制和连续失败计数器，并立即测速切换到健康节点。
