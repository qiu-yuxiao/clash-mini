...
3. **窗口装饰与标题栏**：
   ...
   * **当前活跃出口节点一键轮换功能 (FEAT-006)**：
     ...
     1. 从本地存储中读取当前 Group 的过滤文本、排序方式、正则/大小写匹配等条件。
     2. 对主 Group 的所有节点进行实时过滤与排序，计算出当前在 UI 表格中展示的候选节点子集。
   ...
   * **首次导入自动激活与就绪超时 (BUG-053)**：
    ...
    * **分批测速与首批选点 (BUG-079 / BUG-034 重构)**：
      ...
      * **启动时子集测速选择与唯一全局过滤规则 (BUG-083 & BUG-084)**：
        - **启动时子集过滤有效性 (BUG-083)**：前端自动选点（Profile 切换、启动、Fallback 等场景）通过读取 `proxy-head-state` 获取过滤条件，并使用 `filterSort` 过滤节点子集；后端轻量模式下的自动选点通过读取并解析 `proxy_head_state.json` 实现（详见 **第二十五条**）。前后端均严格遵循用户设置的过滤条件。
