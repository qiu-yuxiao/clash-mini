   ...
   * **界面华丽度双控制系统（双滑动条）**：在“主题模式”下方增加两个滑动条组件（Mui Slider），标签分别为“立体磨砂 (Depth)”与“色彩霓虹 (Vibrancy)”。数值范围为 `0.0` 至 `2.0`（步长为 `0.1`，默认值 `1.0`），数据在 LocalStorage 中持久化保存，并在 React 中监听变化，通过 `document.documentElement.style.setProperty` 将变量 `--depth-factor` 和 `--vibrancy-factor` 动态注入至最顶层 `:root`，用以全局联动控制光影与发光效果。
    ...
    * **多套控件皮肤风格（Skin Swapping）切换支持 (BUG-054)**：
      ...
      * **6种界面美学皮肤**：支持一键切换六种不同的皮肤：
        * `Retro 3D` (3D拟物，默认皮肤，100%完美复刻原有的物理 console 质感，保留所有的 radial 渐变、厚影、凹陷、高光及 hover/active 物理反馈)；
        ...
        * `Frosted Glass` (磨砂玻璃，采用半透明 `rgba` 材质和毛玻璃模糊，剥离拟物 3D 边框，使用细边线和软外阴影，保留极光背景动画)；
      ...
      * **Excel 风格单行选择器与所见即所得**：
        * 换肤选择器采用单行 Excel 单元格拼合表格形式横向排列，格子文字使用各皮肤的展示名称：`Retro-3d`、`Original`、`Modern`、`Frosted`、`Cyberpunk`、`Monochrome`。每个按钮标签使用该皮肤在 `button-styles.ts` 中定义的同款中英文字体族渲染，实现"所见即所得"的字体预览感知。
        * 排列尺寸算法：整个选择器固定总宽度为 `462.5px`。最左侧格子（`Retro-3d`）固定宽度为 `92.5px`，起点定位在偏左 `177.5px`，确保在 `270px` 默认窄窗口下该格精准露出窗口右下角（177.5px 至 270px 区间内）；右侧 5 个格子平分其余 `370px` 空间，每个格子固定宽度为 `74px`。
        * **定位规则**：选择器整体以 `position: absolute; bottom: 0` 紧贴上方设定容器（Upper Pane）底边，仅调整垂直位置，不改变水平对齐。
        * 物理裁剪揭示：在 `270px` 默认窄窗口下，`Retro-3d` 格精准露在窗口右下角，其余 5 个按钮被完全遮挡；拉宽窗口至最大 `640px` 时，其余 5 个按钮向右滑动展示被逐步揭露。
        * 所见即所得渲染：未被选中的单元格仅显示扁平表格文字；一旦点击选中某项，被选中的单元格立刻在此位置以其定义宽度（最左格 `92.5px`，其余格 `74px`）渲染出**该皮肤下的真实物理按钮外观**（如选 `Retro-3d` 渲染为 Radial 黄金 3D 键帽，选 `Original` 渲染为原生扁平按钮）。选中格的圆角不再硬编码为 0px，而是直接继承该皮肤在 `get3DButtonStyle` 中定义的 `border-radius`，从而使 `Original` 等皮肤下的选中格圆角能跟着 Radius 等滑块的变动而动态调节。
      ...
      * **中英文字体共鸣对齐**：
        * 换肤选择器、滑块英文说明以及界面其他按钮文字的英文字体族在切换时与中文字体族进行完美风格对齐：
          * `Retro 3D` 对应：英文 `Trebuchet MS` / 中文 `SimHei`（黑体）；
