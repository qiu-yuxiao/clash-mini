...
6. **多语言支持与翻译规范**：
   ...
   * **三选一分段按钮宽度自适应优化**：为了防止在 240px 宽的设置侧栏中发生按钮文本撑爆或折行：
...
11. **单点测速排序与活跃节点延迟响应规范 (BUG-xxx)**：
    ...
    * **死活 / 显示 / 轮换阈值统一（v2.3.8 收尾）**：前端「活动卡判死 + 红黄绿着色」与「节点列表着色」以及「切下一节点」的轮换判死阈值，一律使用 `NODE_DELAY_MAX_MS(2000)`，与后端死节点判定（`>=2000` 判死）、探针超时（`NODE_TEST_TIMEOUT_MS=2000`）完全一致。单点测速的探针超时（1s / 10s）是独立口径，不受此 2000ms 约束。
...
1. **双层常驻布局**：
   ...
   * 界面不设侧边栏/多页面导航。主视口为垂直 Flexbox 布局 (`flexDirection: column`)。
     ...
     * **视口高度**：自定义标题栏常驻（Windows/Linux 为 frameless 自定义标题栏，高度 30px），视口高度为 `calc(100vh - 30px)`。macOS 使用原生标题栏，视口高度为 `100vh`。隐藏标题栏模式下自定义标题栏隐藏，视口高度为 `100vh`。
     ...
     * **底部指标行 (Metrics Row)**：顶部在普通模式下有 `1px` 边框，顶部外边距为 `4px` (`mt: 0.5`)，顶部内边距为 `4px` (`pt: 0.5`)；极窄模式下隐藏顶部边框，顶部外边距设为 `2px` (`mt: 0.25`)，顶部内边距设为 `0` (`pt: 0`)。指标行内的 4 个指标卡片（上传速度、上传总量、下载速度、下载总量）高度在任何状态下均为 `18px`，采用微凸起、立体按钮感（高 `18px`，圆角 6px，带有上边缘微高光与外阴影投影，拥有与折线图曲线颜色相呼应的背景与边框色）。
       ...
       * 极窄模式 (宽度 $\le 285px$) 下高度为自适应 (`height: auto`)，布局变更为垂直排列 (`flexDirection: column`)，两组之间间距根据模式自适应（`gap: isMinimalWidth ? 0.25 : 0`）。下载组与上传组卡片换行并垂直堆叠展示，高度和填充进一步压缩紧凑化，在超窄视口下避免元素溢出。此外，Tauri 窗口设置中的最小高度限制相应缩减至 `135.0` 物理像素。
...
3. **窗口装饰与标题栏**：
   ...
   * **标题栏行为规范（FEAT-003）**：
     ...
     * **触发条件**：窗口处于**最小宽度（285px）**且**窗口高度小于等于135px**状态，且连续 **10 秒**无任何操作（鼠标移动、点击、键盘）。上述条件须同时满足。

     * **隐藏标题栏模式激活**：隐藏自定义标题栏，GlowBorder 淡入接替窗框的视觉边界。齿轮（设置）和图钉（置顶）按钮在隐藏标题栏模式下继续显示于内容区顶部。窗口总高度不变。

     * **隐藏标题栏模式下移动窗口**：当左键按下且鼠标位移量大于 5 像素时，整个窗口可被拖拽移动，隐藏标题栏状态保持。按钮、链接等交互控件的区域需从拖拽中排除，点击该区域执行其原有功能，不触发窗口移动也不唤醒隐藏标题栏状态。
     ...
     * **Windows 初次启动强制剥除边框样式位并重置尺寸**：Tauri 在 Windows 上创建 `decorations(false)` 窗口时，底层 tao 仅设置 `MARKER_DECORATIONS` 标记，并未从 `GWL_STYLE` 中清除 `WS_CAPTION`。窗口初次创建那一刻 tao 的 `WM_NCCALCSIZE` 子类化尚未安装，Windows 会按 style 自动补上标题栏，导致 outer 尺寸比 `inner_size` 设定的 logical 值多出约 15-22px，初次启动实测出现 300 而非 285。修复方案分两步：① 在 `build_new_window` 的 `builder.build()` 成功之后立即调用 `strip_caption_style(&window)`，用 `SetWindowLongPtrW(GWL_STYLE, ...)` 主动清掉 `WS_CAPTION`（**保留 `WS_THICKFRAME`**——`WS_THICKFRAME` 是 `startResizeDragging` 生效的前提，移除它会导致窗口无法调整尺寸；可见边框由 tao 的 `WM_NCCALCSIZE` 子类化消除，decorations=false 时返回 0 insets），再用 `SetWindowPos(SWP_FRAMECHANGED | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOOWNERZORDER)` 触发一次 `WM_NCCALCSIZE` 重算；② 紧接着调用 `force_set_window_outer_size` 直接用 Win32 `SetWindowPos` 设置 outer 尺寸为 285×680（按 DPI 缩放为物理像素），绕过 tao 的 `set_inner_size`（tao 内部用窗口创建时缓存的 style 算 outer，仍会得到 300）。**注意：修复必须在 `build_new_window` 的 `Ok(window)` 分支里执行，不能放在 `handle_ready_resumed`（RunEvent::Ready），因为 RunEvent::Ready 比窗口创建的异步任务（init_window）早约 0.5 秒触发，此时 `get_webview_window("main")` 返回 None，修复代码被跳过。**

   * 当窗口宽度缩小到极限值 285 px 时，下半层的流量折线图会自动从大图表切换为小图表（`TrafficGraph`），以适应窄窗口下的自适应显示。

   * **自定义内发光边框规范（FEAT-003 附属）**：

     * **定位**：仅在隐藏标题栏模式（hidden titlebar mode）下显示，紧贴原内容区外侧边缘（`position: fixed; inset: 0`），与 Windows 原生窗框互斥交替，接替其视觉边界功能。正常状态下不显示。
...
8. **基础设置模块排版与外观主题**：
   ...
   * **界面华丽度双控制系统（双滑动条）**：在"主题模式"下方增加两个滑动条，标签分别为"立体磨砂 (Depth)"与"色彩霓虹 (Vibrancy)"。数值范围和上限根据不同皮肤进行动态定义（步长为 `0.1`），数据在本地存储中持久化保存，将 CSS 变量 `--depth-factor` 与 `--vibrancy-factor` 动态注入至最顶层，用以全局联动控制光影与发光效果。
    ...
    * **多套控件皮肤风格（Skin Swapping）切换支持 (BUG-054)**：
      ...
      * **特殊保留限制**：
        * 换肤修改范围严格局限于主题皮肤切换。中缝分割双线 `5px double`、设置页 `4px double` 双线边框及表格 `2px` 网格线无论在何种皮肤下均保持现状，不准改动；无边框隐藏标题栏状态下最外圈的 GlowBorder 采用静态设计（降低 CPU/GPU 渲染负载至 0%），且样式随着选定的皮肤风格进行自适应切换（如 retro-3d 使用金色渐变双线、cyberpunk 使用荧光渐变等）。
...
- **精确防丢包抖动的自愈时间参数**：
  ...
  - **前后端探针超时与判死阈值统一（v2.3.6 收紧）**：探针超时 == 判死阈值 == `NODE_DELAY_MAX_MS(2000ms)`。后端 `NODE_TEST_TIMEOUT_MS` 由宽松的 `10000ms` 收紧为等于 `NODE_DELAY_MAX_MS`；前端 F4（per-group 闪电按钮）与 chain-mode 的 `checkListDelay` 探针超时也统一为 `NODE_DELAY_MAX_MS(2000)`。超过 `2000ms` 的节点一律按不可用处理，UI 显示 `Error`/`Timeout`，不再展示其真实延迟（精度换一致性）。
- **自愈选点失败冷却**：
...
- **API 异常不可静默忽略**：
  - `check_active_node_health` 返回 `Err`（mihomo API 不可达，如 `get_group_by_name` 超时/连接失败）时，日志级别必须为 **`warn`**（不可用 `debug`），确保在 normal 日志级别可见。
  - 必须维护 `api_error_count` 连续异常计数器，每次 `Err` 累加，`Ok` 时清零。
  - 连续 **`3次`** API 异常（正常模式约 45 秒，重试模式约 9 秒）后，必须触发一次自愈选点尝试恢复。若自愈也因 API 不可达而失败，`auto_select_fail_count` 会累计，5 次后弹 Windows 警报。
...
- **自动选点成功后必须回写 profile.selected**：
  - 后端 `trigger_backend_auto_select` 在 `select_node_for_group("PROXY", ...)` 成功后，必须同步调用 `profiles_patch_item_safe` 将选中节点写回 `profile.selected`。
  - 否则唤醒/进入轻量模式时 `restore_profile_selected_nodes` 与 `activateSelected` 会用过时的 `selected.now` 把节点切回旧节点（可能是已不在当前过滤条件子集内的节点），导致 PROXY 组真实切换到错误节点几秒到十几秒。
  - `selected` 数组只含 PROXY 一项：`[{ name: 'PROXY', now: '新节点名' }]`。
  - 前端无需重复实现回写逻辑，所有 `triggerAutoSelect` 调用路径（批量测速、唤醒测速、F1 自动选点）均自动受益。
...
- **本地连接池空闲管道激进清空**：
  ...
  - 这将在轻量模式下，彻底断开并关闭全局连接池中保留的 3 个空闲本地管道连接（NamedPipe），消除最后几处系统活跃句柄的占用。
