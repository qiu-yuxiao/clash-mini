   ...
   * **界面华丽度双控制系统（双滑动条）**：在“主题模式”下方增加两个滑动条组件（Mui Slider），标签分别为“立体磨砂 (Depth)”与“色彩霓虹 (Vibrancy)”。数值范围为 `0.0` 至 `2.0`（步长为 `0.1`，默认值 `1.0`），数据在 LocalStorage 中持久化保存，并在 React 中监听变化，通过 `document.documentElement.style.setProperty` 将变量 `--depth-factor` 和 `--vibrancy-factor` 动态注入至最顶层 `:root`，用以全局联动控制光影与发光效果。
    ...
    * **多套控件皮肤风格（Skin Swapping）切换支持 (BUG-054)**：
      ...
      * **6种界面美学皮肤**：支持一键切换六种不同的皮肤：
        ...
        * `Original` (原版致敬，从左数第二位，完全复制原版默认配色与扁平排版布局，彻底剥离 Bevel 3D 立体厚度和渐变、aurora 极光背景动画及背光外发光，呈现原汁原味的经典 Clash Verge 风格)；
...
15. **STYLE-001 3D 物理拟物设计规范与参数说明 (参考 AIMP Euphoria Lux NY 风格)**：
    ...
    * **全应用 24 个交互式与展示型拟物控件清单**：
      ...
      | **3D 浮雕展示卡片** | **5 个** | 1. 主页第一行：“当前活跃出口节点”状态卡片<br>2. 主页最底行：4 个流量监控指标数据卡片<br>3. 设置左侧分栏 5 个模块卡片（订阅与机场配置、流量接管模式、分流策略倾向、基础设置、主题设置） | 抽象并应用 `get3DCardStyle` 统一管理，支持同心圆径向渐变。在 `Trump-3D` 皮肤下，boxShadow 必须与按钮保持一致：**3 层无模糊 bevelShadowDark 固态挤压像素层叠 + 单层接触投影 + 单层顶高光 + 可选霓虹外发光**。切忌降级为纯模糊阴影方案（BUG-117）。 |
...
7. **订阅激活自动链式动作 (BUG-034)**：
   ...
   * **首次导入自动激活与就绪超时 (BUG-053)**：
    ...
    * **自动选点非阻塞与临时闪连 (BUG-079 / BUG-034 优化)**：
      ...
      * **内核就绪防崩**：内核启动检测的 `refreshProxy` 必须包装在 `try-catch` 中，防止端口未准备好时网络请求直接抛错中断自动选点流程。

      * **非阻塞测速触发**：触发 Provider 测速（`healthcheckProxyProvider`）或 inline 节点测速（`checkListDelay`）时，禁止 `await` 同步等待其完全返回，必须在后台异步执行，防止网络连接差或节点多导致被同步阻塞数分钟。

      * **临时闪连**：轮询开启后，一旦检测到第一个健康可用节点（延迟 > 0 且 < 1e6），即静默切换（`changeProxy`）到该节点，不弹出提示打扰用户，确保用户在 1~2 秒内立即可用网络。

      * **极速终选与提前终止**：以下任一条件满足时，进行最终测速完成切换并弹出通知，并退出轮询：
        - 已有 5 个健康可用节点（大样本已够）。
        - 已测出延迟或测速超时的节点数等于过滤后的有效节点总数（专为小节点子集设计，避免为未响应死节点空等）。
        - 轮询时间达到 15 秒（上限防死锁）。
        - 若最终没有发现任何健康节点，弹出“所有线路都繁忙”友好提示。
...
8. **多订阅并行与全自动链接格式识别 (Parallel Multi-Subscription & Auto Format Detection)**：
   ...
   * **本地零泄漏通用解析与自动识别**：
     ...
     * 在下载或更新订阅链接时，后端自动检测链接内容格式：优先尝试以标准 YAML 格式解析；如果 YAML 校验失败，自动尝试对链接内容进行解码（Base64 或 raw 明文 URI 列表，支持 Shadowsocks (`ss://`)、Trojan (`trojan://`)、VMess (`vmess://`)、VLESS (`vless://`)、Hysteria2 (`hysteria2://`) 格式，以及特殊 `[HTTP]` 兜底格式），将其解析为代理结构体并转换为 valid YAML 保存至本地对应的 R/L 订阅文件中。若均无法解析，则抛出详细的格式错误，在边界安全拦截。
...
## ⚡ 二十四、 手动触发全体测速后自动选中最快健康节点规范 (BUG-092)
为了解决测速后用户需要手动查找并点击最快节点的不便，实现测速与选路的一体化闭环，制定以下规范：
- **目标候选范围限制（过滤后的子集）**：
  - 自动选路操作的目标节点集必须严格限制在**用户当前在 UI 列表中看到的过滤后候选节点子集（Filtered Subset）**。即：仅对当前界面搜索输入框、延迟过滤或类型过滤后显现的节点进行挑选，严禁在未显示的隐藏节点中进行选择。
- **最快健康节点判定标准**：
  - 测速完成后，遍历当前过滤出的节点子集。
  - 获取每个节点的最新延迟（通过 `delayManager.getDelayFix`）。
  - 有效健康节点的延迟必须满足：`delay > 0` 且 `delay < timeout`（其中 `timeout` 为代理组配置的默认延迟超时上限）。
  - 从所有健康节点中挑选延迟数值最小（最快）的节点。若没有节点满足该条件，则保持当前选中的节点不变。
- **自动切换执行**：
  - 若找到符合条件的最快健康节点，自动调用 `changeProxy(groupName, fastestNodeName, currentActiveNodeName)` 执行策略组出口节点切换，从而同步更新 Clash 内核选路、托盘菜单状态并持久化本地选择。

## ⚡ 二十五、 后端常驻活跃节点监控与自动选点规范 (BUG-083/093)
为了保证客户端在无人值守、系统重启、进入轻量模式或前端关闭等各种状态下，均能实现代理节点 24/7 自动健康监测、失效自愈及正则过滤条件的持久化记忆，制定以下规范：
- **前端输入决定，后端记忆执行**：
  - 前端界面所有的搜索、正则过滤、测速与手动选择节点状态，都应通过统一的 Tauri 命令 `save_proxy_head_state` 实时传递并持久化保存至后端的配置文件 `proxy_head_state.json` 中。
  - 后端 Rust 守护线程（Daemon Loop）在运行和自动选点时，必须读取并严格遵循该持久化文件中的正则过滤条件，确保不管开机关机或前端关闭，过滤条件在后台始终生效。
- **两阶段内核与 API 就绪检查**：
  - 为了防止在配置切换或内核加载期间发生误报，监控线程必须先等待本地 Clash API就绪，并在获取代理信息接口响应后，循环请求并确认当前活跃 Profile 的节点列表（`all` 数组）长度大于 0。若超过 10 秒仍未加载完成，则进行安全退出或重试，防止提前判定失效而误触发切换。
- **并发受限的后台批量并行测速**：
  - 后端在自动触发选点或手动触发测速时，通过使用 tokio 异步机制和并发上限控制器（如并发上限锁定为 32），向本地 Clash API 并发发起节点延迟测试，兼顾测速效率与系统 CPU、I/O 的稳定性。
- **精确防丢包抖动的自愈时间参数**：
  - **检测周期**：常规情况下以 **`15秒`** 的周期进行单点检测；一旦发生故障，自动切换至 **`3秒`** 的高频重试模式。
  - **防抖自愈触发阈值**：必须连续 **`3次`** 检测失败（整个周期跨度约为 7.5 秒，刚好避开了运营商小于 5 秒的常见网络路由抖动/震荡期）才触发批量选点自愈。
  - **测速超时限制**：活跃节点单点检测超时设为 **`500ms`**，只要成功返回即为健康；全节点优选测速超时设为 **`2000ms`**，快速剔除卡顿/死节点。
- **网络连接的防死锁与参数转义安全**：
  - **客户端超时**：所有的后端监控 API 客户端必须配置 **`3秒` 的显式 Timeout 限制**，防止内核挂起导致异步监控线程无限期阻塞（死锁）。
  - **URL 编码防转义**：测速接口拼接 `test_url` 时，必须预先进行 **URL 百分号编码（Percent-Encoding）**，防止 `:`、`/`、`?` 破坏内核请求参数解析。
- **无竞态冲突的前端缓存同步（useHeadStateNew）**：
  - 前端 `useHeadStateNew` 钩子必须在组件挂载时一次性载入完整的全配置过滤词字典，仅在内存状态发生修改时写入 LocalStorage 与后端文件，禁止在 Profile 切换时在 Effect 中进行异步网络请求，彻底消除切换 Profile 时的写入覆盖风险。
- **无效正则的平滑降级**：
  - 在后端使用 `regex::Regex` 解析过滤正则条件时，如遇用户输入的不完整或无效正则（导致 Regex 解析报错），必须平滑降级为普通的子串包含（`contains`）匹配，防止后台线程崩溃。
- **后台自动选点静默机制**：
