...
2. **验证脚本（`verify.py`）的硬性拦截限制**：
   - 验证脚本在解析 `bug_list.md` 底部的历史归档表格时，若发现当前即将发布的版本号（由 `package.json` 读取）下存在任何状态不是 `代码已修正，已确认` 的条目（例如仍为 `待用户确认`），必须报错并以 `sys.exit(1)` 直接拦截构建。
   - 此机制旨在硬性阻止 AI 助手在发布新版本前，通过强行塞入历史表格但保留“待确认”状态的手段来规避 changelog 校验。
