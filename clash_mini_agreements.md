...
1. **双层常驻布局**：
   ...
   * 界面不设侧边栏/多页面导航。主视口为垂直 Flexbox 布局 (`flexDirection: column`)。
     ...
     * **视口高度**：自定义标题栏常驻（Windows/Linux 为 frameless 自定义标题栏，高度 36px），视口高度为 `calc(100vh - 36px)`。macOS 使用原生标题栏，视口高度为 `100vh`。隐身模式下自定义标题栏隐藏，视口高度为 `100vh`。
...
3. **窗口装饰与标题栏**：
   ...
   * **标题栏行为规范（FEAT-003）**：
     ...
     * **默认状态**：Windows/Linux 启动时为 frameless（无原生装饰），显示自定义标题栏（36px 高度，底部 1px 分割线，右侧最小化/最大化/关闭按钮）。macOS 使用原生标题栏。标题栏文字统一使用标准 ASCII 英文字符，例如 `"Clash Mini Ver.1.1.1"`，避免字形缺失导致乱码。

     * **窗口缩放**：frameless 模式下通过 8 个方向的透明 resize handle（6px 宽）实现边缘拖拽缩放，最大化时自动隐藏。

     * **触发条件**：窗口处于**最小宽度（270px）**且**窗口高度小于等于100px**状态，且连续 **10 秒**无任何操作（鼠标移动、点击、键盘）。上述条件须同时满足。
