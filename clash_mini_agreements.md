...
2. **滑出设置面板**：
   ...
   * 设置面板内采用**左右分栏**布局：
      * **左侧分栏（固定为 200px 宽度）**：自上而下整齐摆放「机场订阅区」、「代理模式选择」与「极简设置项」，不随窗口宽度变化拉伸。其中，为了解决标题字号反而显得小的问题，抽屉内所有设置项标签（如“开机自动启动”、“启动时最小化”、“通知弹窗显示”、“主题模式”、“代理策略”、“Mixed Port”等）的字号一律统一为 `11px`，与各自模块标题字号（`11px`）保持一致。同时，为了使三个开关按钮（开机自动启动、启动时最小化、通知弹窗显示）与小文字视觉比例协调，Switch 控件采用 `transform: 'scale(0.75)'` 进行整体缩小，并采用 `transformOrigin: 'right center'` 靠右对齐。
...
5. **唯一代理组与去机场规则化 (Single Proxy Group & Anti-Airport Rules)**：
   ...
   * **GLOBAL 组的同步绑定与联动保障**：不管处于何种代理模式下，系统在界面和逻辑上表现为仅有唯一的节点列表（即 `PROXY` 组下平铺的所有物理节点）。为确保在「全局代理」模式下，流量出口依然 100% 同步跟随用户在主列表里选中的节点，后台及前端逻辑必须强制将 `GLOBAL` 组绑定指向唯一代理组 `PROXY`（通过 `selectNodeForGroup('GLOBAL', 'PROXY')` 保证在启动、配置文件更新重载、以及代理模式切换时强行生效）。用户在界面上只需切换 `PROXY` 组的节点，流量出口在任何模式下均自动跟随改变，绝不允许因 `cache.db` 的历史缓存或其他操作导致 `GLOBAL` 组与 `PROXY` 组发生脱节。
6. **开机自动启动与 UAC 提权机制**：
