...
为了降低程序在轻量模式（托盘后台运行）时的系统资源占用、不必要的网络流量和电池消耗，并兼顾前台运行时的用户体验，制定以下自适应检测周期规范：

- **自适应检测时间间隔**：
  - **前台活动模式**：当客户端处于非轻量模式（主窗口显示或处于活动状态）时，常驻后台检测线程的健康探测周期采用标准的 **`15` 秒**，以保证网络断流时能及时发现并进行故障自愈。
  - **轻量后台模式**：当客户端进入轻量模式（主窗口销毁，程序仅以系统托盘常驻后台）时，探测周期自适应自动延展至 **`60` 秒**，以极大地减少后台 CPU/电池唤醒频率和 DNS 探测流量。
  - **故障重试模式**：无论前台还是后台，一旦监测到活跃节点出现健康度检查异常，探测周期统一切换为 **`3` 秒** 进行高频快速重试。在连续失败 5 次触发自动优选切换后，若失败则按照退避算法进行指数冷却，若成功则重置并回归对应前后台的正常周期。
- **退出轻量模式的即时唤醒补偿**：
  - 在用户从系统托盘重新唤醒/重建主窗口并退出轻量模式时，必须通过全局信号量 **`MONITOR_WAKEUP_NOTIFY`** 立即唤醒正在处于 60 秒长休眠状态的监测线程。
  - 线程唤醒后，重新评估当前轻量状态并回归 15 秒短间隔周期，确保主界面重新显现后活跃节点的监控状态处于最新、最灵敏的水平，杜绝界面恢复后因等待长休眠导致的延迟更新滞后现象。
