    ...
    * **多套控件皮肤风格（Skin Swapping）切换支持 (BUG-054)**：
      ...
      * **滑块功能及范围限制（以现存前端代码为准）**：
        * 两个滑动条的标签显示必须为**纯英文**，且其底层控制参数在不同皮肤下被完全重定义，并各自在本地存储（localStorage）中建立独立的字段分别保存配置，避免数值冲突覆盖：
          * `Trump-3D` (retro-3d) 皮肤：滑块一显示为 `Depth`（范围 `0.0` - `1.0`），滑块二显示为 `Vibrancy`（范围 `0.0` - `5.0`）；
...
- **自适应健康检测周期**：
  ...
  - **活跃状态（Normal）**：监测周期维持常规的 **15秒**，确保在用户正在操作前端界面时，节点故障自愈足够灵敏。
  - **轻量/后台挂机状态（Lightweight/In）**：监测周期自动放宽至 **60秒**，以最大化降低后台挂机时的系统资源开销。
...
#### 3. TextField 必须有关联的标签

禁止使用 `hiddenLabel` + `placeholder` 替代标签。所有 `<TextField>` 必须通过 `label` 属性（推荐）或 `slotProps.htmlInput['aria-label']`（仅视觉设计不允许标签时使用）提供可访问名称。
...
// ⚠️ 仅限无可见标签空间时
<TextField
  slotProps={{ htmlInput: { 'aria-label': t('shared.placeholders.filter') } }}
  placeholder={t('shared.placeholders.filter')}
/>

// ❌ 错误
<TextField hiddenLabel placeholder="过滤条件" />
```

**引用标准**: WCAG 2.2 3.3.2 Labels or Instructions (Level A)
