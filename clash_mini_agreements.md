...
3. **窗口装饰与标题栏**：
   ...
   * **标题栏行为规范（FEAT-003）**：
     ...
     * **Windows 初次启动强制剥除边框样式位并重置尺寸**：Tauri 在 Windows 上创建 `decorations(false)` 窗口时，底层 tao 仅设置 `MARKER_DECORATIONS` 标记，并未从 `GWL_STYLE` 中清除 `WS_CAPTION | WS_THICKFRAME`。窗口初次创建那一刻 tao 的 `WM_NCCALCSIZE` 子类化尚未安装，Windows 会按 style 自动补上 resize 边框（SM_CXSIZEFRAME + SM_CXPADDEDBORDER），导致 outer 尺寸比 `inner_size` 设定的 logical 值多出约 15-22px（DPI 96 时约 +15，DPI 144 时约 +22），初次启动实测出现 300 而非 285。修复方案分两步：① 在 `handle_ready_resumed` 中调用 `strip_caption_thickframe_style(&window)`，用 `SetWindowLongPtrW(GWL_STYLE, ...)` 主动清掉 `WS_CAPTION | WS_THICKFRAME`，再用 `SetWindowPos(SWP_FRAMECHANGED | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOOWNERZORDER)` 触发一次 `WM_NCCALCSIZE` 重算，让 tao 的无边框处理立即生效；② 清除 style 后窗口尺寸仍是创建时被 `AdjustWindowRectEx` 算出的 outer≈300px，需紧接着调用 `window.set_size(LogicalSize::new(DEFAULT_WIDTH, DEFAULT_HEIGHT))` 将窗口尺寸重置回 285×680，否则会变成"300px 的无边框窗口"而非"285px 的无边框窗口"。
