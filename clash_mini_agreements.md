...
3. **窗口装饰与标题栏**：
   ...
   * **标题栏行为规范（FEAT-003）**：
     ...
     * 窗口启动时的默认尺寸硬性设定为：宽度 270 px，高度 680 px。同时，支持拉伸，最大尺寸限制设定为：宽度 640 px，高度 860 px；最小尺寸限制设定为：最小宽度 270 px，最小高度 129 px。

     * **窗口最小尺寸强制约束**：窗口在任何情况下不得缩小至小于最小尺寸（270×129 px），包括用户拖拽缩放操作中。最小化和最大化状态下不生效。
   ...
   * **自定义内发光边框规范（FEAT-003 附属）**：
     ...
     * **宽度**：**4px**。GlowBorder 外置于内容区边缘，不占用内容区空间，**窗口尺寸无需为其调整**（DEFAULT_WIDTH=270, MINIMAL_WIDTH=270, MINIMAL_HEIGHT=129）。
