...
- **测速过程中的行为与反馈**：
  ...
  - **全局并发互斥**：`DelayManager.checkListDelay` 入口必须检查 `_isBatchTesting` 标志，若已有批量测速正在进行（无论来自前台闪电按钮，还是后台 profile 切换测速、6 秒 Fallback 测速、窗口唤醒测速），后续请求直接丢弃并返回。理由：所有测速目标均为 PROXY 组节点，正在进行的测速会覆盖相同数据，丢弃策略无增量损失，且可避免前后台测速叠加产生 72 路以上并发 IPC 撑爆主线程导致卡死。组件层 `testingGroups` 状态仅能防用户重复点击同一组闪电，无法拦截后台测速入口，因此互斥必须在 `DelayManager` 层实现。

- **最快健康节点判定与自动切换**：
  - **最快健康节点判定**：测速全部完成后，遍历这组过滤后的可见节点，获取其最新延迟。有效且健康的节点延迟必须大于等于 `30ms` 且小于当前代理组设定的超时上限。
