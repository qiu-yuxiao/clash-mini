...
- **BUG-270 解决轻量模式下托盘菜单勾号缺失及新窗口创建死锁卡死问题**：
  - **前缀字符状态栏标识**：在轻量模式下动态为菜单项文本前缀增加 `✓` 字符标识开启状态，退出时自动移除，解决常规菜单项无法展示对勾的问题。
  - **消除 UI 主线程死锁**：在托盘菜单事件回调中将窗口激活与创建的操作延迟调度在主线程上（使用 `app.run_on_main_thread`），彻底消除 Windows 原生菜单循环与后台异步创建窗口之间的跨线程死锁冲突，防止窗口被拉起时产生假死。
