...
*   **你必须**：
    ...
    3. **以方案 and 直观结果交付**：必须通过 `implementation_plan.md` 进行顶层设计对齐，并在交付时通过清晰 of `walkthrough.md` 汇报测试结果与静态检查状态（如编译输出、类型校验）。
*   **你严禁**：
...
*   **触发场景**：当你执行任何终端命令（如文件/目录删除、npm包安装、git操作或自定义脚本运行）时。
*   **你必须**：
    1. 确保所有命令在非交互模式（Non-interactive）下运行，预先传递静默、强制、自动接受（如 `-Recurse -Force`、`/s /q`、`-y`、`--non-interactive`）的参数。
    2. 对任何可能由于目录非空、文件被锁、网络证书或环境不一致而引发的确认提示（Y/N），提前在命令中进行规避或强力覆盖。
*   **你严禁**：
    1. 严禁运行任何未经强力静默处理、且在异常分支下可能挂起等待标准输入（stdin）的交互式命令。

## 🛑 第十一条：跨目录访问前置授权律 (Law of Preemptive Out-of-Bounds Permission)
*   **触发场景**：当因排查故障或获取环境上下文，需要访问当前工作区根目录之外的其他本地路径（如外部代理路径、系统配置路径）时。
*   **你必须**：
    1. 在对该越界目录执行任何读取、修改或命令操作前，**必须首先**调用 `ask_permission` 工具对该目录的**父级文件夹（Parent Directory）**申请一次性递归授权。
    2. 申请原因（Reason）中需清晰、明确地向用户说明跨目录调查的必要性，并等待用户批准。
    3. 授权通过后，若需要运行脚本或指令分析该外部目录的数据，必须在工作区内部（Cwd 设定在当前工作区内）执行命令，并在命令参数中指向该已授权的外部路径，利用本地脚本与外部授权目录的配合实现高效静默执行。
*   **你严禁**：
    1. 严禁不经过前置父目录授权，直接使用逐个文件点对点读取的方式，避免在执行过程中触发系统对单个文件的重复弹窗确认，打扰用户。
