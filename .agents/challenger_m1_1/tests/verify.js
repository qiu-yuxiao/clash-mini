...
const jiti = createJiti(import.meta.url, {
  alias: {
    ...
    'tauri-plugin-mihomo-api': path.resolve(__dirname, 'mocks/tauri-plugin-mihomo-api.js'),
    '@tanstack/react-query': path.resolve(__dirname, 'mocks/tanstack-react-query.js'),
    'foxact/use-local-storage': path.resolve(__dirname, 'mocks/foxact-use-local-storage.js'),
    '@/services/cmds': path.resolve(__dirname, 'mocks/services-cmds.js'),
    './use-clash-log': path.resolve(__dirname, 'mocks/use-clash-log.js'),
    './use-traffic-monitor': path.resolve(__dirname, 'mocks/use-traffic-monitor.js'),
    './use-mihomo-ws-subscription': path.resolve(__dirname, 'mocks/use-mihomo-ws-subscription.js'),
  }
});

// Import mock modules for controlling their states
const { resetReactMock, setRenderCallback, runEffects, runHook } = await jiti.import('./mocks/react.js');
const { windowState, resetWindowState } = await jiti.import('./mocks/tauri-window.js');
const { queryClient } = await jiti.import('./mocks/tanstack-react-query.js');
const { subscriptionCalls, clearSubscriptionCalls } = await jiti.import('./mocks/use-mihomo-ws-subscription.js');

// Import the real hooks under test
const { useVisibility } = await jiti.import('../../../src/hooks/use-visibility.ts');
const { useTrafficData } = await jiti.import('../../../src/hooks/use-traffic-data.ts');
...
globalThis.document = {
  ...
  removeEventListener: (event, cb) => {
    if (documentListeners[event]) {
      documentListeners[event] = documentListeners[event].filter(c => c !== cb);
    }
  }
};
...
async function runTests() {
  ...
  assert(visResult.current === true, 'Initially, document visible and not minimized -> useVisibility is true');

  // Change document visibility to hidden
  globalThis.document.visibilityState = 'hidden';
  triggerDocumentEvent('visibilitychange');
  runEffects();
  assert(visResult.current === false, 'Document visibility becomes hidden -> useVisibility is false');

  // Change document visibility back to visible
  globalThis.document.visibilityState = 'visible';
  triggerDocumentEvent('visibilitychange');
  runEffects();
  assert(visResult.current === true, 'Document visibility becomes visible -> useVisibility is true');
  ...
  await sleep(10);
  runEffects();
  assert(visResult.current === false, 'Window minimized -> useVisibility is false');
  ...
  await sleep(10);
  runEffects();
  assert(visResult.current === true, 'Window restored (not minimized) -> useVisibility is true');
  ...
  assert(subscriptionCalls.length > 0, 'useTrafficData registers a subscription call');
  let latestCall = subscriptionCalls[subscriptionCalls.length - 1];
  let key = latestCall.buildSubscriptKey(12345);
  assert(key === 'getClashTraffic-12345', `Subscription key initially active: "${key}"`);
  ...
  assert(key === null, `Subscription key correctly set to null when visibility is false`);
  ...
  assert(subscriptionCalls.length > 0, 'useLogData registers a subscription call');
  latestCall = subscriptionCalls[subscriptionCalls.length - 1];
  key = latestCall.buildSubscriptKey(12345);
  assert(key === 'getClashLog-12345', `Subscription key initially active: "${key}"`);
