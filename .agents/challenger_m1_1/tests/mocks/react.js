...
export function useState(initialVal) {
  ...
  const setState = (newVal) => {
    const nextVal = typeof newVal === 'function' ? newVal(stateObj.value) : newVal;
...
export function useEffect(effect, deps) {
  ...
  const hasChanged = !oldHook || !deps || deps.some((d, i) => d !== oldHook.deps[i]);
...
export function runEffects() {
  ...
  for (const item of queue) {
    if (item.cleanup) {
      try { item.cleanup(); } catch(e){}
    }
  }
  // Execute effects and save cleanup functions
  for (const item of queue) {
    const cleanup = item.effect();
    hookStates[item.index].cleanup = typeof cleanup === 'function' ? cleanup : null;
  }
}
